    ...
    if (type%in%c('joint',0,1,3)){
        if ('a0'%ni%inames){
            if (anchor[1]==1){
                ...
                } else if (x$format%in%c(7,8,11,85,115)){
...
fixDispersion <- function(model,format,anchor,type){
    ...
    if (model==3 & anchor[1]==1){
        ...
        } else if (format%in%c(7,8,11,12,85,115,1210)){
            if (type==1){
...
LL.ludwig <- function(par,x,X=x,model=1,exterr=FALSE,anchor=0,type='joint'){
    ...
    if ('a0' %in% pnames){
        ...
        a0 <- ifelse(anchor[1]==3,
                     sk[1,'i64'],
                     iratio('Pb206Pb204')[1])
    } else if (x$format%in%c(7,8,11,85,115) & type%in%c('joint',0,1,3)){
        a0 <- ifelse(anchor[1]==3,
                     sk[1,'i64']/sk[1,'i84'],
                     iratio('Pb206Pb208')[1])
    }
    ...
    } else if (x$format%in%c(7,8,11,12,85,115,1210)){
        if (anchor[1]==1){
...
getEw1 <- function(w=0,x,McL=mclean(),type,a0=NA,b0=NA,c0){
    ...
    } else if (format%in%c(4,5,6,9,10,85,115,1210)){
        if (type==1){
...
data2ludwig.2d <- function(ta0b0w,x,model=1,exterr=FALSE,type=1,anchor=0){
    ...
    } else if (x$format%in%c(4,5,6,9,10,85,115,1210)){
...
mswd.lud <- function(fit,x,exterr=FALSE,type='joint'){
    ...
    if (x$format%in%c(1,2,3,9,10,115,1210) || type%ni%c('joint',0)){
        out$df <- ns-np
    } else {
        out$df <- 2*ns-np
    }
