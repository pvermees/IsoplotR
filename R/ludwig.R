...
ludwig2d <- function(x,type=1,model=1,anchor=0,exterr=FALSE){
    if (x$format %in% (4:6)) {
        ...
        logparnames <- c('log(t)','log(64i)','log(74i)')
    } else if (x$format %in% (7:8)) {
        parnames <- c('t','68i','78i')
        logparnames <- c('log(t)','log(68i)','log(78i)')
    } else {
        stop("Illegal input format.")
    }
    if (anchor[1]==0) {
        ...
        lta0b0 <- anchored.lta0b0.init(x=x,anchor=anchor)
    }
    if (type%in%c(1,3)){
        ij <- c(1,2) # 3D parameters to override
        init <- lta0b0[-3]
    ...
    if (model==3){
        lw <- optimize(LL.lud.2d.w,interval=c(-10,7),
                       lta0=init,x=x,type=type)$minimum
        init <- c(init,lw)
        ij <- c(ij,4)
        parnames <- c(parnames,'w')
        logparnames <- c(logparnames,'log(w)')
    }
    NP <- length(fixed)+1
    np <- sum(!fixed)
    lower <- (init-1)[!fixed]
    upper <- (init+1)[!fixed]
    if (model==2) fn <- SS.model2.2d
    else fn <- LL.lud.2d
    fit <- optifix(init,fixed=fixed,fn=fn,
                   x=x,type=type,method="L-BFGS-B",
                   lower=lower,upper=upper,hessian=TRUE)
    out <- list(model=model,logpar=rep(NA,NP),logcov=matrix(0,NP,NP),
                par=rep(NA,NP),cov=matrix(0,NP,NP))
    out$logpar[ij] <- fit$par
    out$logcov[ij[!fixed],ij[!fixed]] <- solve(fit$hessian)
    names(out$logpar) <- logparnames
    rownames(out$logcov) <- logparnames
    colnames(out$logcov) <- logparnames
    ...
    SS <- LL.lud.2d(lta0w=fit$par,x=x,type=type,LL=FALSE)
    out$n <- length(x)
    out$df <- out$n-np
    out$p.value <- as.numeric(1-stats::pchisq(SS,out$df))
    out$mswd <- SS/out$df
...
LL.lud.2d <- function(lta0w,x,type=1,LL=TRUE){
    model3 <- (length(lta0w)>2)
    tt <- exp(lta0w[1])
    if (model3) w <- exp(lta0w[3])
    if (type%in%(1:2)){
        ...
        l2 <- lambda('Th232')[1]
        Pb208Th232 <- age_to_Pb208Th232_ratio(tt)[1]
        dPb208Th232dt <- l2*exp(l2*tt)
        x0 <- 1/Pb208Th232
        y0 <- exp(lta0w[2])
    } else {
    ...
    if (model3){
        Ew <- matrix(0,2*ns,2*ns)
        Ew[cbind(i1,i1)] <- (w*xy[,1]/x0)^2
        covmat <- covmat + Ew
    }
    if (LL){
...
LL.lud.2d.w <- function(w,lta0,x,type=1){
    lta0w <- c(lta0,w)
    LL.lud.2d(lta0w=lta0w,x=x,type=type)
}

SS.model2.2d <- function(lta0,x,type=1){
    tt <- exp(lta0[1])
    a0 <- exp(lta0[2])
    ns <- length(x)
    if (x$format<7) xy <- get.UPb.isochron.ratios.204(x)
    else xy <- get.UPb.isochron.ratios.208(x,tt=tt)
    if (type==1){
        X <- xy[,1] # U238Pb206
        Y <- xy[,2] # Pb204Pb206 or Pb208cPb206
        D <- mclean(tt,d=x$d)
        x0 <- 1/D$Pb206U238
        y0 <- 1/a0
    } else if (type==2){
        X <- xy[,3] # U235Pb207
        Y <- xy[,4] # Pb204Pb207 or Pb208cPb207
        D <- mclean(tt,d=x$d)
        x0 <- 1/D$Pb207U235
        y0 <- 1/a0
    } else if (type==3){
