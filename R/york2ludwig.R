...
york2ludwig204 <- function(x,anchor=0,type=0,buffer=2,model=1){
    ...
    if (anchor[1]==1){
        ...
        if (type%in%c('joint',0,1)){
            ...
            tst <- get.Pb206U238.age(x=abxa['x0inv'],d=x$d)
            par['t'] <- log(tst[1])
        ...
        if (type==2){
            tst <- get.Pb207U235.age(x=abxb['x0inv'],d=x$d)
        }
        par['t'] <- log(tst[1])
        if (type%in%c('joint',0,1)){
            par['a0'] <- log(1/abxa['a'])
    ...
    if ('b0'%in%names(par)){
        lower['b0'] <- par['b0'] - buffer
        upper['b0'] <- par['b0'] + buffer
    }
    if ('w'%in%names(par)){
        lower['w'] <- par['w'] - max(buffer,10)
...
york2ludwig208 <- function(x,anchor=0,type=0,buffer=2,model=1){
    ...
    if (anchor[1]==1){
        ...
        if (type==1){
            ...
            if (model==1 & iratio('Pb206Pb208')[2]>0){
                par['a0'] <- -log(y0)
            }
        } else if (type==2){
            par['t'] <- log(get.Pb207U235.age(x=abx['x0inv'],d=x$d)[1])
            if (model==1 & iratio('Pb207Pb208')[2]>0){
                par['b0'] <- -log(y0)
            }
        } else if (type==3){
