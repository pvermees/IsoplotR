...
age_to_wetherill_ratios <- function(tt,st=0,exterr=FALSE,diseq=FALSE,
    ...
    if (diseq){
        ...
        dD1dt <- dd1dt(tt=tt,D0=Pa1U5)
        dD2dt <- dd2dt(tt=tt,A0=U48,B0=Th0U8,C0=Ra6U8)
        dD1dl5 <- dd1dl5(tt=tt,D0=Pa1U5)
        dD2dl8 <- dd2dl8(tt=tt,A0=U48,B0=Th0U8,C0=Ra6U8)
    } else {
        D1 <- 0
        D2 <- 0
        dD1dt <- 0
    ...
    Pb7U5 <- exp(l5*tt)-1 + D1
    Pb6U8 <- exp(l8*tt)-1 + D2
    out$x <- c(Pb7U5,Pb6U8)
    E <- matrix(0,3,3)
    diag(E) <- c(st,lambda('U235')[2],lambda('U238')[2])^2
    J <- matrix(0,2,3)
    J[1,1] <- l5*exp(l5*tt) + dD1dt
    J[2,1] <- l8*exp(l8*tt) + dD2dt
    if (exterr){
        J[1,2] <- tt*exp(l5*tt) + dD1dl5
        J[2,3] <- tt*exp(l8*tt) + dD2dl8
    }
    out$cov <- J %*% E %*% t(J)
    names(out$x) <- labels
    rownames(out$cov) <- labels
...
age_to_terawasserburg_ratios <- function(tt,st=0,exterr=FALSE,
    ...
    Pb207U235 <- exp(l5*tt)-1 + D1
    Pb206U238 <- exp(l8*tt)-1 + D2
    U238Pb206 <- 1/Pb206U238
    Pb207Pb206 <- (1/U)*Pb207U235/Pb206U238
    d75dt <- l5*exp(l5*tt) + dD1dt
    d68dt <- l8*exp(l8*tt) + dD2dt
    d75dl5 <- tt*exp(l5*tt) + dD1dl5
    d68dl8 <- tt*exp(l8*tt) + dD2dl8   
    out$x <- c(U238Pb206,Pb207Pb206)
    E <- matrix(0,4,4)
...
age_to_U238Pb206_ratio <- function(tt,st=0,diseq=FALSE,
    ...
    R <- 1/(exp(l8*tt)-1 + D2)
    J <- -(l8*exp(l8*tt) + dD2dt)/(exp(l8*tt)-1 + D2)^2
    R.err <- abs(J*st)
    out <- cbind(R,R.err)
...
get.Pb207U235.age.default <- function(x,sx=0,exterr=TRUE,
    ...
    J <- matrix(0,1,2)
    if (diseq){
        t.75 <- optim(t.75,diseq.75.misfit,x=x,D0=Pa1U5)$par
        D1 <- d1(tt=tt,D0=Pa1U5)
        dD1dt <- dd1dt(tt=tt,D0=Pa1U5)
        xe1d <- x-exp(l5*t.75)+1-D1 # misfit = f = xe1d^2
        dfdx <- 2*xe1d
        dfdt <- 2*xe1d*(-l5*exp(l5*t.75)-dD1dt)
        J[1,1] <- -dfdx/dfdt        # dt/dx
        if (exterr){
            dD1dl5 <- dd1dl5(tt=tt,D0=Pa1U5)
            dfdl5 <- 2*xe1d*(-t.75*exp(l5*t.75)-dD1dl5)
...
get.Pb206U238.age.default <- function(x,sx=0,exterr=TRUE,diseq=FALSE,
    ...
    J <- matrix(0,1,2)
    if (diseq){
        t.68 <- optim(t.68,diseq.68.misfit,x=x,
                      A0=U48,B0=Th0U8,C0=Ra6U8)$par
        D2 <- d2(tt=tt,A0=U48,B0=Th0U8,C0=Ra6U8)
        dD2dt <- dd2dt(tt=tt,A0=U48,B0=Th0U8,C0=Ra6U8)
        xe1d <- x-exp(l8*t.68)+1-D2 # misfit = f = xe1d^2
        dfdx <- 2*xe2d
        dfdt <- 2*xe2d*(-l8*exp(l8*t.68)-dD2dt)
        J[1,1] <- -dfdx/dfdt        # dt/dx
        if (exterr){
            dD2dl8 <- dd2dl8(tt=tt,A0=U48,B0=Th0U8,C0=Ra6U8)
            dfdl8 <- 2*xe1d*(-t.68*exp(l8*t.68)-dD2dl8)
...
get.Pb207Pb206.age.default <- function(x,sx=0,exterr=TRUE,diseq=FALSE,
                                       U48=1,Th0U8=0,Ra6U8=0,Pa1U8=0,...){
    if (is.na(x)){
        t.76 <- NA
    } else {
        t.76 <- stats::optimize(Pb207Pb206.misfit,c(0,1/l8),x=x,
                                diseq=diseq,U48=U48,Th0U8=Th0U8,
                                Ra6U8=Ra6U8,Pa1U8=Pa1U8)$minimum
    }
    J <- matrix(0,1,4)
    DMFDT <- dmf76dt(x,t.76,diseq=diseq,A0=U48,B0=Th0U8,C0=Ra6U8,D0=Pa1U8)
    J[1,1] <- -(-1)/DMFDT
    if (exterr){
        J[1,2] <- -dmf76dl5(x,t.76,diseq=diseq,A0=U48,B0=Th0U8,C0=Ra6U8,D0=Pa1U8)/DMFDT
        J[1,3] <- -dmf76dl8(x,t.76,diseq=diseq,A0=U48,B0=Th0U8,C0=Ra6U8,D0=Pa1U8)/DMFDT
        J[1,4] <- -dmf76dU(x,t.76,diseq=diseq,A0=U48,B0=Th0U8,C0=Ra6U8,D0=Pa1U8)/DMFDT
    }
    E <- matrix(0,4,4)
    E[1,1] <- sx^2
...
get.Pb207Pb206.age.UPb <- function(x,i,exterr=TRUE,diseq=FALSE,
    ...
    get.Pb207Pb206.age(r76[i,'Pb207Pb206'],r76[i,'errPb207Pb206'],exterr=exterr,
                       diseq=diseq,U48=U48,Th0U8=Th0U8,Ra6U8=Ra6U8,Pa1U8=Pa1U8)
}
get.Pb207Pb206.age.wetherill <- function(x,exterr=TRUE,diseq=FALSE,
    ...
    get.Pb207Pb206.age(r76,sr76,exterr=exterr,diseq=diseq,
                       U48=U48,Th0U8=Th0U8,Ra6U8=Ra6U8,Pa1U8=Pa1U8)
}
...
dd1dl5 <- function(tt,D0){
    l5 <- settings('lambda','U235')[1]
    d1(tt,D0)*(tt+1/l5)
}
dd2dl8 <- function(tt,A0,B0,C0){
    l8 <- settings('lambda','U238')[1]
    d1(tt,A0,B0,C0)*(tt+1/l8)
}
dd1dl5 <- function(tt,D0){
    l5 <- settings('lambda','U235')[1]
    l1 <- settings('lambda','Pa231')[1]
    l5*d1(tt,D0) + D0*l5*exp((l5-l1)*tt)
}
dd2dl8 <- function(tt,A0,B0,C0){
    l6 <- settings('lambda','Ra226')[1]
    l0 <- settings('lambda','Th230')[1]
...
diseq.75.misfit <- function(x,tt,D0){
    l5 <- settings('lambda','U235')[1]
    misfit <- (x - exp(l5*tt) + 1 - d1(tt=tt,D0=D0))^2
}
diseq.68.misfit <- function(x,tt,A0,B0,C0){
    l8 <- settings('lambda','U238')[1]
    misfit <- (x - exp(l8*tt) + 1 - d2(tt=tt,A0=A0,B0=B0,D0=D0))^2
}
Pb207Pb206.misfit <- function(tt,x,diseq=FALSE,U48=1,Th0U8=0,Ra6U8=0) {
    (age_to_Pb207Pb206_ratio(tt,diseq=diseq,U48=U48,
                             Th0U8=Th0U8,Ra6U8=Ra6U8)[,'76']-x)^2
}
# derivative of the 7/6 misfit function w.r.t. time
dmf76dt <- function(x,t.76,diseq=FALSE,A0=1,B0=0,C0=0,D0=0){
    l5 <- lambda('U235')[1]
    l8 <- lambda('U238')[1]
    U <- iratio('U238U235')[1]
    if (diseq){
        D1 <- d1(t.76,D0=D0)
        D2 <- d2(t.76,A0=A0,B0=B0,C0=C0)
        dD1dt <- dd1dt(t.76,D0=D0)
        dD2dt <- dd2dt(t.76,A0=A0,B0=B0,C0=C0)
    } else {
        D1 <- 0
        D2 <- 0
        dD1dt <- 0
    ...
    r75 <- exp(l5*t.76)-1+D1
    r68 <- exp(l8*t.76)-1+D2
    dr75dt <- l5*exp(l5*t.76)+dD1dt
    dr68dt <- l8*exp(l8*t.76)+dD2dt
    2*(r75/(U*r68)-x)*(dr75dt*r68-r75*dr68dt)/(U*r68^2)
}
dmf76dl5 <- function(x,t.76,diseq=FALSE,A0=1,B0=0,C0=0,D0=0){
    l5 <- lambda('U235')[1]
    l8 <- lambda('U238')[1]
    U <- iratio('U238U235')[1]
    if (diseq){
    ...
    r75 <- exp(l5*t.76)-1+D1
    r68 <- exp(l8*t.76)-1+D2
    dr75dl5 <- t.76*exp(l5*t.76)+dD1dl5
    2*(r75/(U*r68)-x)/(U*r68)
}
dmf76dl8 <- function(x,t.76,diseq=FALSE,A0=1,B0=0,C0=0,D0=0){
    l5 <- lambda('U235')[1]
    l8 <- lambda('U238')[1]
    U <- iratio('U238U235')[1]
    if (diseq){
        D1 <- d1(t.76,D0=D0)
        D2 <- d2(t.76,A0=A0,B0=B0,C0=C0)
        dD2dl8 <- dd2dl8(t.76,A0=A0,B0=B0,C0=C0)
    } else {
        D1 <- 0
        D2 <- 0
        dD2dl8 <- 0
    ...
    r75 <- exp(l5*t.76)-1+D1
    r68 <- exp(l8*t.76)-1+D2
    dr68dl8 <- l8*exp(l8*t.76)+dD2dt
    2*(r75/(U*r68)-x)*(-r75*dr68dl8)/(U*r68^2)
}
dmf76dU <- function(x,t.76,diseq=FALSE,A0=1,B0=0,C0=0,D0=0){
    l5 <- lambda('U235')[1]
    l8 <- lambda('U238')[1]
    U <- iratio('U238U235')[1]
    if (diseq){
