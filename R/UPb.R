...
get.UPb.isochron.ratios <- function(x,i){
    ...
    if (x$format==4){ # 75, 68, 48
        ...
        Pb204Pb206 <- x$x[i,'Pb204Pb206']
        U235Pb207 <- x$x[i,'U238Pb206']/(U*x$x[i,'Pb207Pb206'])
        ...
        J[1,1] <- 1
        J[2,3] <- 1
        J[3,1] <- 1/(U*x$x[i,'Pb207Pb206'])
        J[3,2] <- -U235Pb207/x$x[i,'Pb207Pb206']
...
age_to_U235Pb207_ratio <- function(tt,st=0,d=diseq()){
    ...
    if (ns>1){
        ...
        l5 <- lambda('U235')[1]
        D <- wendt(tt=tt,d=d)
        R <- 1/(exp(l5*tt)-1 + D$d1)
        J <- -R*(l5*exp(l5*tt) + D$dd1dt)/(exp(l5*tt)-1 + D$d1)
        R.err <- abs(J*st)
        out <- cbind(R,R.err)
        colnames(out) <- c('57','s[57]')
    }
