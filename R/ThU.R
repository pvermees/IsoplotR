...
get.ThU.age.corals <- function(x,exterr=FALSE,i=NA,sigdig=NA,
    ...
    if (detritus==1)
        d <- Th230correction.isochron(d,dat=x,omit=omit)
    else if (detritus==3)
...
get.ThU.age.volcanics <- function(x,exterr=FALSE,i=NA,i2i=FALSE,sigdig=NA,omit=NULL){
    ns <- length(x)
    d <- data2york(x,type=2,generic=FALSE)
    if (i2i){
        fit <- isochron.ThU(x,type=2,plot=FALSE,exterr=FALSE,omit=omit)
