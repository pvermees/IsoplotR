...
init_ludwig <- function(x,anchor=0,buffer=2,type=0,model=1){
    if (anchor[1]==3){ # stacey-kramers
        init <- init_ludwig(x=x,anchor=0,buffer=buffer,type=type,model=model)
        out <- list(par=init$par['t'],
                    lower=init$lower['t'],
                    upper=init$upper['t'])
    } else if (x$format<4){
        out <- init_ludwig_TW(x=x,anchor=anchor,buffer=buffer,model=model)
    } else if (x$format%in%c(4,5,6,9,10,85,119,1210)){
        out <- init_ludwig_20x(x=x,anchor=anchor,buffer=buffer,type=type,model=model)
    } else if (x$format%in%c(7,8,11,12)){
        out <- init_ludwig_208(x=x,anchor=anchor,buffer=buffer,type=type,model=model)
    } else {
        stop("Invalid U-Pb format")
    }
    if (x$d$U48$option==2 | x$d$ThU$option==2){
        if ('t'%in%names(out$par)) tt <- exp(out$par['t'])
    ...
    if (type%in%c('joint',0,1,3)){
        if (x$d$U48$option==1 & x$d$U48$sx>0){
            out$par['U48i'] <- x$d$U48$x
        } else if (x$d$U48$option==2 & x$d$U48$sx>0){
            out$par['U48i'] <- max(x$d$buffer,McL$U48i)
        }
        if ('U48i'%in%names(out$par)){
            out$lower['U48i'] <- x$d$U48$m + x$d$buffer
            out$upper['U48i'] <- x$d$U48$M - x$d$buffer
        }
        if (x$d$ThU$option==1 & x$d$ThU$sx>0){
            out$par['ThUi'] <- x$d$ThU$x
        } else if (x$d$ThU$option==2 & x$d$ThU$sx>0){
            out$par['ThUi'] <- max(x$d$buffer,McL$ThUi)
        }
        if ('ThUi'%in%names(out$par)){
            out$lower['ThUi'] <- x$d$ThU$m + x$d$buffer
            out$upper['ThUi'] <- x$d$ThU$M - x$d$buffer
        }
        if (x$d$RaU$option==1 & x$d$RaU$sx>0){
            out$par['RaUi'] <- x$d$RaU$x
            out$lower['RaUi'] <- x$d$RaU$m + x$d$buffer
            out$upper['RaUi'] <- x$d$RaU$M - x$d$buffer
        }
    }
    if (type%in%c('joint',0,2,4)){
        if (x$d$PaU$option==1 & x$d$PaU$sx>0){
            out$par['PaUi'] <- x$d$PaU$x
            out$lower['PaUi'] <- x$d$PaU$m + x$d$buffer
            out$upper['PaUi'] <- x$d$PaU$M - x$d$buffer
        }
    }
    out
}
...
init_ludwig_20x <- function(x,anchor=0,type=0,buffer=2,model=1){
    ...
    if (anchor[1]==1){
        if (type%in%c('joint',0,1)){
            ...
            abxa <- init_ludwig_helper(yd=yda,y0=1/Pb6xc[1])
        ...
        if (type%in%c('joint',0,2)){
            ...
            abxb <- init_ludwig_helper(yd=ydb,y0=1/Pb7xc[1])
        }
        if (type==2){
        ...
        if (type%in%c('joint',0,1)){
            ...
            abxa <- init_ludwig_helper(yd=yda,x0=1/Pb6U8r)
            par['a0'] <- log(1/abxa['a'])
        }
        if (type%in%c('joint',0,2)){
            Pb7U5r <- mclean(tt=tt)$Pb207U235
            abxb <- init_ludwig_helper(yd=ydb,x0=1/Pb7U5r)
            par['b0'] <- log(1/abxb['a'])
        }
        if (model==3 & length(anchor)>2 && anchor[3]<=0){
            if (type==1){
        ...
        if (type%in%c('joint',0,1)){
            abxa <- init_ludwig_helper(yd=yda)
            tt <- get_Pb206U238_age(x=abxa['x0inv'],d=x$d)[1]
        }
        if (type%in%c('joint',0,2)){
            abxb <- init_ludwig_helper(yd=ydb)
        }
        if (type==2){
            tt <- get_Pb207U235_age(x=abxb['x0inv'],d=x$d)[1]
        }
...
init_ludwig_208 <- function(x,anchor=0,type=0,buffer=2,model=1){
    par <- lower <- upper <- vector()
    if (anchor[1]==1){
        ...
        abx <- init_ludwig_helper(yd=yd,y0=y0)
        if (type==1){
        ...
        abx <- init_ludwig_helper(yd=yd,x0=x0)
        if (type==1){
            par['a0'] <- log(1/abx['a'])
        } else if (type==2){
            par['b0'] <- log(1/abx['a'])
            ...
            abxb <- init_ludwig_helper(yd=ydb,x0=x0)
            par['b0'] <- log(1/abxb['a'])
        }
        if (model==3 & length(anchor)>2 && anchor[3]<=0){
            if (type%in%c('joint',0,1)){
