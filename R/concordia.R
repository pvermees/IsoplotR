...
get.concordia.limits <- function(x,tlim=NULL,type=1,xlim,ylim,...){
    ...
    if (measured.disequilibrium(x$d)){
        ...
        if (!is.null(tlim) & type==1){
            ...
            out$t[1] <- get.Pb206U238.age(1/maxx,d=median(x$d))[1]
            out$t[2] <- get.Pb207Pb206.age(maxy,d=median(x$d),
                                           interval=c(out$t[1],10000))[1]
            if (!xset)
                minx <- min(minx,age_to_U238Pb206_ratio(out$t[2],d=median(x$d))[,'86'])
            if (!yset)
                miny <- min(miny,age_to_Pb207Pb206_ratio(out$t[1],d=median(x$d))[,'76'])
            out$x <- c(minx,maxx)
            out$y <- c(miny,maxy)
        } else if (is.null(tlim) & type==3){
            if (!xset){
            ...
            out$t[1] <- get.Pb206U238.age(minx,d=median(x$d))[1]
            out$t[2] <- get.Pb208Th232.age(maxy)[1]
            if (!xset){
                minx <- min(minx,age_to_Pb206U238_ratio(out$t[1],d=median(x$d))[,'68'])
                maxx <- max(maxx,age_to_Pb206U238_ratio(out$t[2],d=median(x$d))[,'68'])
            }
            if (!yset){
