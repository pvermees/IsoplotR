...
matrix2covlist <- function(x){
    ...
    covmat <- matrix(0,3,3)
    for (i in 1:ns){
        covmat[c(1:2),c(1:2)] <- cor2cov(x[i,2],x[i,4],x[i,7])
        covmat[c(1,3),c(1,3)] <- cor2cov(x[i,2],x[i,6],x[i,8])
        covmat[c(2:3),c(2:3)] <- cor2cov(x[i,4],x[i,6],x[i,9])
...
fisher.tit <- function(abAB,dat){
    ...
    for (i in 1:ns){
        ...
        d2L.dx2 <- -(omega[1,2] + 2*b*omega[1,2] + 2*B*omega[1,3] +
