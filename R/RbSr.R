...
ID.Rb <- function(x,exterr=FALSE,isochron=TRUE){
...
get.RbSr.age <- function(Sr87Rb87,sSr87Rb87,exterr=TRUE){
    l87 <- lambda('Rb87')
    tt <- log(1 + Sr87Rb87)/l87[1]
    E <- matrix(0,2,2)
    J <- matrix(0,1,2)
    E[1,1] <- sSr87Rb87^2
    if (exterr) E[2,2] <- l87[2]^2
    J[1,1] <- 1/(l87[1]*(1 + Sr87Rb87))
    J[1,2] <- -tt/l87[1]
    st <- sqrt(J %*% E %*% t(J))
    c(tt,st)
}
