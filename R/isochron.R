...
isochron.UPb <- function(x,alpha=0.05,sigdig=2, show.numbers=FALSE,
    ...
    D <- mclean(tt,d=median(x$d))
    if (type==1){                           # 04-08c/06 vs. 38/06
        x0inv <- age_to_Pb206U238_ratio(tt=tt,st=0,d=median(x$d))[1]
        dx0invdt <- D$dPb206U238dt
        E <- lud$cov[1:2,1:2]
        x.lab <- quote(''^238*'U/'^206*'Pb')
    } else if (type==2){                    # 04-08c/07 vs. 35/07
        x0inv <- age_to_Pb207U235_ratio(tt=tt,st=0,d=median(x$d))[1]
...
isochron.KCa <- function(x,alpha=0.05,sigdig=2,show.numbers=FALSE,
    ...
    isochron_PD(x,nuclide='K40',alpha=alpha,sigdig=sigdig,
                ...
                ci.col=ci.col,line.col=line.col,lwd=lwd,plot=plot,
                exterr=exterr,model=model,show.ellipses=show.ellipses,
                bratio=0.895,hide=hide,omit=omit,omit.fill=omit.fill,
                omit.stroke=omit.stroke,...)
}
#' @rdname isochron
#' @export
